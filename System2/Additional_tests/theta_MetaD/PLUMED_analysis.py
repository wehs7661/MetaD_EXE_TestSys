...
def logger(*args, **kwargs):
    print(*args, **kwargs)
    abspath = '/home/wei-tse/Documents/MetaD_EXE_TestSys/System2/Additional_tests/theta_MetaD'
    with open(f"{abspath}/PLUMED_analysis_results.txt", "a") as f:
        print(file=f, *args, **kwargs)

if __name__ == '__main__':
    k_constant = ['k_original', 'k_20', 'k_200']
    state = ['state_0', 'state_8']
    for i in k_constant:
        for j in state:
            logger(f'========== Data analysis of {i}/{j} ==========')
            os.chdir(f'{i}/{j}')
            if os.path.isdir('analysis') is True:
                os.system('rm -r analysis')
            os.system('mkdir analysis')

            # Task 1
            logger('Generating fes.dat and plotting the free energy profile ...')
            os.system('plumed sum_hills --hills HILLS --bin 100')
            os.system(f'plot_2d -f fes.dat -x "Torsional angle relevant to the H.B. (degree)" -y "Free energy (kT)" -t "Torsional angle as a function of time" -n fes_{i}_{j} -cx "radian to degree" -cy "kJ/mol to kT"')
            ...
            for k in range(len(f_min)):
                logger(f'Local minimum {k + 1}: at theta = {theta_min[k]}, f = {f_min[k]} (closest time frame: at {t_min[k]} ps, theta = {CV[diff.index(min(diff))]})')
            ...
            os.system(f'plot_2d -f COLVAR -y "Torsional angle relevant to the H.B. (degree)" -x "Time (ps)" -t "Torsional angle as a function of time" -n dihedral_{i}_{j} -cy "radian to degree"')
            os.system('mv *fes* *png *min* analysis')
            os.chdir('../../')
