...
if __name__ == '__main__':
    ...
    for i in range(len(files)):
        ...
        if i < len(files) - 1:
            plt.plot(theta, fes, label=f'{0.5 * (i + 1)} ns', linestyle='--')
        else:
            plt.plot(theta, fes, label=f'{0.5 * (i + 1)} ns')
    ...
    plt.grid()
    #plt.legend()
    plt.savefig('sys2_fes_angle_max_ext.png', dpi=600)
    ...
    plt.grid()
    plt.tight_layout()
    plt.savefig('sys2_mininfo_angle_max_ext.png', dpi=600)
