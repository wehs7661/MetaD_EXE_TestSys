...
def plot_all_time_series(data_dir, fig_name):
    ...
    for i in range(n_subplots):
        f_list = files[2 * i : 2 * (i + 1)]  # f_list[0]: dihedral_min_1, f_list[1]: dihedral_min_2
        plt.subplot(n_rows, n_cols, i + 1)
        time, theta = get_time_series(f_list[0])
        plt.plot(time, theta, label='Min 1', alpha=0.5)

        time, theta = get_time_series(f_list[1])
        plt.plot(time, theta, label='Min 2', alpha=0.5)
        ...
        plt.ylabel(f'{a_str} (deg)', fontsize=8)
    ...
    plt.suptitle(f'The time series of all {a_type}s', fontsize=12, weight='bold')
...
if __name__ == '__main__':
    t1 = time.time()
    trajs = ['../../state_A/2D_simulation/rep_1/sys2.xtc', '../../state_B/lambda_MetaD/2D_simulation/rep_1/sys2.xtc']
    top = '../../dihedral_min_1/lambda_MetaD/2D_simulation/rep_1/sys2.top'
    ...
    plot_all_time_series('angle', 'Sys2_remodified_sys2_angle_timeseries.png')
    plot_all_time_series('dihedral', 'Sys2_remodified_sys2_dihedral_timeseries.png')
