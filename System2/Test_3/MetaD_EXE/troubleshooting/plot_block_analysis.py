...
if __name__ == '__main__':
    ...
    for i in range(20):
        ...
        f = open(f'rep_{i + 1}/block_analysis_result_truncate_0.8.txt', 'r')
        lines = f.readlines()
        f.close()
    ...
    plt.ylim([14.5, 18])
    plt.legend(ncol=4)
    plt.savefig('delta_f_20_reps_truncate_0.8.png', dpi=600)
    ...
    for i in range(20):
        ...
        plt.plot(0.02 * block_size, delta_f_err, label=f'Rep {i + 1}')
        sigma.append(delta_f_err[49])  # block_size = 50 ps
    
    plt.legend(ncol=4)
    plt.savefig('delta_f_err_20_reps_truncate_0.8.png', png=600)

    # Figure 3: Distribution
    x = np.arange(10, 20.01, 0.01)
    plt.figure()
    ...
    for i in range(20):
        ...
        plt.legend(ncol=2)
    plt.savefig('delta_f_distribution_20_reps_truncate_0.8.png', png=600)
