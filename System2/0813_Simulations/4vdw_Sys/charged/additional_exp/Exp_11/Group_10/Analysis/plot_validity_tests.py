...
if __name__ == "__main__":
    ...
    for i in range(5):
        plt.plot(range(1, 7), p_values[i], label=labels[i])
    ax.set_yscale('log')
    plt.xlabel('Time interval')
    plt.ylabel('p-values')
    plt.title('Experimental 11, Group 10')
    plt.grid()
    plt.legend()
    ...
    ax = fig.add_subplot(1, 1, 1)
    for i in range(5):
        plt.errorbar(range(1, 7), T_mu[i], yerr=T_mu_err[i], label=labels[i])
    plt.xlabel('Time interval')
    plt.ylabel('T_mu')
    plt.title('Experimental 11, Group 10')
    plt.grid()
    ...
    ax = fig.add_subplot(1, 1, 1)
    for i in range(5):
        plt.errorbar(range(1, 7), T_sigma[i], yerr=T_sigma_err[i], label=labels[i])
    plt.xlabel('Time interval')
    plt.ylabel('T_sigma')
    plt.title('Experimental 11, Group 10')
    plt.grid()
