...
if __name__ == "__main__":
    ...
    for i in range(len(bools)):
        for j in range(5):
            ...
            edr='../sys2_cccc.edr',
            trr=f'sys2_cccc_lig_{i}.trr'
            )

            res.system.ndof_reduction_tra = 0
            
            with open(f'results_{test_type[i]}_{times[i]}_{times[j]}.txt', 'a') as f:
                with contextlib.redirect_stdout(f):
                    results = physical_validation.kinetic_energy.equipartition(
                        data=res,
                        strict=bools[i],
                        filename=f'exp9_group3_{times[j]}_{times[j + 1]}',
                    )
                    f.write(results)
