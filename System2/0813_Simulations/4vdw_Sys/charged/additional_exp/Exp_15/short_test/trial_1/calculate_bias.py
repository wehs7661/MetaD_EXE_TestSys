...
def initialize():
    ...
    parser.add_argument(
        ...
        "--period",
        nargs="+",
        default=["2 * np.pi", "None"],
        help="The period of the CVs (e.g. [2 * np.pi, None])."
    )
    parser.add_argument(
        "-g",
        "--grid_info",
        nargs="+",
        default=["-np.pi", "np.pi", "361", "0", "15", "16"],
...
def gaussian(x, mu, sigma, period=None):
    """
    ...
    if period is None:
        ...
        diff = min(np.abs(x - mu), period - np.abs(x - mu))
        g = np.exp(-diff ** 2 / (2 * sigma ** 2))
    return g
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(grid_points))):
        ...
        for j in range(len(hills_data["time"])):   # sum up all the multivariate Gaussians
            ...
            for k in range(n_CVs):  # get the Gaussian for each dimnsion
                g_list.append(gaussian(grid_points[i][k], mu_list[k][j], sigma_list[k][j], eval(args.period[k])))
    ...
    CV_str = '\t'.join(args.vars)
    fes_file.write(f"\t{CV_str}\tf\n")
