...
def logger(*args, **kwargs):
    print(*args, **kwargs)
    with open("sum_hills_log_redo.txt", "a") as f:
        print(file=f, *args, **kwargs)

def save_data(*args, **kwargs):
    with open("sum_hills_data_redo.txt", "a") as f:
        print(file=f, *args, **kwargs)
...
if __name__ == '__main__':
    ...
    start = 600000   # 200000 Gaussians, 2000000 steps, 4 ns
    ...
    for i in track(range(4001)):    # 100 * 500 = 50000 Gaussians, 1 ns
        t = (start + step * i) * 0.02
    ...
    logger("Reading sum_hills_data_redo.txt")    
    data = np.loadtxt("sum_hills_data_redo.txt")
