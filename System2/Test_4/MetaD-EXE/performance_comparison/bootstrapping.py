...
def block_bootstrap(traj, n_blocks, dt, file_path, B, T=298.15, truncate=0, CV='lambda'):
    ...
    for i in range(int(np.max(traj[f"{CV}"])) + 1):
        ...
        """
        print('A = ', isState[i][boot])
        print('B = ', isState[i])
        print(len(isState[i][1]))
        from sys import getsizeof
        print(round(getsizeof(isState) / 1024 / 1024,2))
        print(round(getsizeof(isState[i]) / 1024 / 1024,2))
        print(round(getsizeof(isState[i][boot]) / 1024 / 1024,2))
        print(len(isState))
        print(len(isState[i][boot]))
        print(len(isState[i]))
        print(np.average(isState[i][boot], axis=(1, 2), weights=w[boot]))
        """
