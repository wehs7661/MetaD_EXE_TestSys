...
def initialize():
    parser = argparse.ArgumentParser(
        description='This code uses block bootstrap to calculate the free energy \
                     difference between the coupled and uncoupled state from an \
                     alchemical metadynamics. Example command: python bootstrapping.py \
                     -d rep_1 -n 10 100 200 500 1000 2000 5000 -t 0.2')
...
def block_bootstrap(traj, n_blocks, dt, file_path, B, T=298.15, truncate=0):
    """
    ...
    fes_file.write('lambda    f             f_err\n')
    isState, pop, fes, fes_err = [], [], [], []
    for i in range(int(np.max(traj["lambda"])) + 1):
        # 1 if in state i
        isState.append(np.int_(traj["lambda"] == i)
        ...
        fes_err.append(np.std(fes_boot))
...
def metad_bias(hills):
    """
    Calculates the total bias as a function of lambda.

    Parameters
    ----------
    hills (PlumedDataFrame): The data read from HILLS. 

    Returns
    ------- 
    bias  (np.array): the total bias as a function of lambda.
    """
    bias = []
    for i in range(np.max(hills["lambda"] + 1)):
        bias.append(np.sum(np.array(hills["lambda"] == i, dtype=int)
                           * hills["height"] / hills["biasf"] * (hills["biasf"] - 1)))
    bias = np.array(bias)
    return bias
...
if __name__ == '__main__':
    ...
    for folder in args.dir:
        ...
        if args.seed is not None:
            ...
            args.seed = 'None'  # just for logging
        ...
        if avg_frac == 0:
            print('====================== Reminder ======================')
            print('Reminder: The final bias is used for reweighting.')
        ...
        for i in range(len(n_blocks)):
            ...
            df_results.append(block_bootstrap(
                traj, n_blocks[i], args.factor, fes_path, B, T, truncate))
    ...
    if len(args.dir) > 1 and len(n_blocks) == 1:
        ...
        L.logger(f'- Standard deviation of all reps: {np.std(df_list): .6f}')
        if args.ref is None:
            args.ref = np.mean(df_list)
        rmse = np.sqrt(np.sum((np.array(df_list) - args.ref) ** 2))
