...
def average_bias(hills, avg_frac):
    ...
    """
    hills_unphysical = pd.concat([hills[hills.n.gt(10)], hills[hills.n.lt(1)]])
    hills_physical = hills[hills.n.between(1, 10)]
    # hills_unphysical = hills[hills.n.gt(10)]
    # hills_physical = hills[hills.n.le(10)]
    n_reweighted = int(len(hills_physical) * avg_frac)
    
    n_list = []
    n_list.append(len(hills))                  # total number of points
    n_list.append(len(hills[hills.n.gt(10)]))   # number of points with N > 10
...
if __name__ == "__main__":
    ...
    for folder in args.dir:
        ...
        L.logger(f'- Number of points in the unphysical region (N > 10): {n_list[1]} ({n_list[1] / n_list[0] * 100:.2f}%)')
        # L.logger(f'    - Number of points with N > 10: {n_list[1]} ({n_list[1] / n_list[0] * 100:.2f}%)')
        # L.logger(f'    - Number of points with N < 1: {n_list[2]} ({n_list[2] / n_list[0] * 100:.2f}%)')
        L.logger(f'- Number of points in the physical region (N <= 10): {n_list[0] - n_list[1]} ({100 - n_list[1] / n_list[0] * 100:.2f}%)')
        L.logger(f'- Number of points for reweighting: {n_list[2]}\n')
