...
def initialize():
    ...
    parser.add_argument('-t',
                        ...
                        type=float,
                        default=0,
                        help='The fraction of the simulation to be truncated from the end.')
    ...
    parser.add_argument('-f',
                        ...
                        type=float,
                        default=0.02,
                        help='The factor for converting the block size into ps / 1 COLVAR stride in ps.')
    parser.add_argument('-T',
                        '--temp',
                        type=float,
                        default=298.15,
...
class Logging:
    ...
    def logger(self, *args, **kwargs):
        """
        Prints the results on screen and to output file.
...
def average_bias(hills, avg_frac):
    ...
    """
    hills_unphysical = pd.concat([hills[hills.n.gt(7)], hills[hills.n.lt(1)]])    
    hills_physical = hills[hills.n.between(1, 7)]
    n_reweighted = int(len(hills_physical) * avg_frac)
    
    n_list = []
    n_list.append(len(hills))                  # total number of points
    n_list.append(len(hills[hills.n.gt(7)]))   # number of points with N > 7
    n_list.append(len(hills[hills.n.lt(1)]))   # number of points with N < 1
    n_list.append(n_reweighted)                # number of points for reweigting
    
    idx_ignored = hills_unphysical.index
    idx_reweighted = hills_physical.index[-n_reweighted:]
    ...
    for i in range(len(hills)):
        if i in idx_ignored:
            w[i] = 0
        elif i in idx_reweighted:
            n += 1 
            w[i] = np.linspace(1, 0, n_reweighted)[n]
    hills_avg = hills.copy()
    hills_avg.height *= w
    return hills_avg, n_list
...
if __name__ == "__main__":
    ...
    for folder in args.dir:
        ...
        # Case 2: Using the time-averaged bias for reweighting (args.avg > 0)
            ...
            mem_avg, hills_result = memory_usage(
            ...
            plumed.write_pandas(hills_result[0], args.hills + "_modified")
            n_list = hills_result[1]
        ...
        L.logger(f'- Total number of points (in the HILLS file): {n_list[0]}')
        L.logger(f'- Number of points in the unphysical region: {n_list[1] + n_list[2]} ({(n_list[1] + n_list[2]) / n_list[0] * 100:.2f}%)')
        L.logger(f'    - Number of points with N > 7: {n_list[1]} ({n_list[1] / n_list[0] * 100:.2f}%)')
        L.logger(f'    - Number of points with N < 1: {n_list[2]} ({n_list[2] / n_list[0] * 100:.2f}%)')
        L.logger(f'- Number of points in the physical region: {n_list[0] - n_list[1] + n_list[2]} ({100 - (n_list[1] + n_list[2]) / n_list[0] * 100:.2f}%)')
        L.logger(f'- Number of points for reweighting: {n_list[3]}\n')
