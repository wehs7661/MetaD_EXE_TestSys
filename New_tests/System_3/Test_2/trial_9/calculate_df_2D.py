...
def block_bootstrap(traj, n_blocks, dt, file_path, B, T=298, truncate=0, CV='lambda'):
    """
    ...
    np.save('weights.npy', np.exp(bias / kT)[-n:])
...
if __name__ == "__main__":
    ...
    for folder in args.dir:
        ...
        analysis_data = plumed.read_as_pandas('analysis.dat')
        traj = traj[::100]  # to have the same frequency as analysis_data
        traj = traj.reset_index()   # to have the same index ans analysis_data
        traj = traj.drop(['index'], axis= 1)
        traj = traj[~((analysis_data['d'] > 0.45803763) & (analysis_data['lambda']==0))]
