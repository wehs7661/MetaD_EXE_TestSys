...
def block_bootstrap(traj, n_blocks, dt, file_path, B, T=298.15, truncate=0, CV='lambda'):
    ...
    b1 = np.array(traj["metad_1.bias"])  
    b2 = np.array(traj["metad_2.bias"])
    b3 = np.array(traj['uwall.bias'])
    b4 = np.array(traj['lwall.bias'])
    bias = b1 + b2 + b3 + b4
